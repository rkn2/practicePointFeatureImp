...
pairplot_src = [
    ...
    "g = sns.pairplot(df[['Avg_Temp_C','Temp_Range_C','Annual_Rainfall_mm','Humidity_Percent','Soil_Moisture_Index','Crack_Width_mm','Condition_Rating']])\n",
    "# Explicitly turn off grid for all subplots in pairplot\n",
...
factor_scatter_src = [
    ...
    "# Re-define fa_vars if needed or use the columns used for FA\n",
    "# In the notebook, the data for FA is likely 'data_for_fa' or similar\n",
    "# We will try to use 'data_for_fa' which should be defined in the notebook\n",
    "try:\n",
    "    scores = fa.transform(data_for_fa)\n",
    "except NameError:\n",
    "    # Fallback if variable name is different, try to reconstruct or use df columns\n",
    "    # Assuming 'df' and the columns are available\n",
    "    cols = ['Avg_Temp_C', 'Temp_Range_C', 'Annual_Rainfall_mm', 'Humidity_Percent', 'Freeze_Thaw_Cycles', 'Soil_Moisture_Index', 'Crack_Width_mm', 'Salt_Deposition_g_m2']\n",
    "    scores = fa.transform(df[cols])\n",
    "\n",
...
print('All cells appended with grid=False')
